        ...
        <v-list-item>
          ...
          <div class="textAreaSideBar">
            <input
              ...
              v-if="editTask == true"
              :disabled="editTask"
              @keyup.enter="updateGroupTaskName"
            />
            <input
              maxlength="49"
              type="text"
          ...
          <v-dialog v-model="taskDialog" max-width="380">
            <v-card>
              <div class="popupConfirmHeadline">
                ...
                >You're about to permanantly delete this task, its comments and attachments, and all of its data. If you're not sure, you can cancel this action.</span>
              </div>
      ...
      <v-list flat>
            ...
            <v-list-item-content>
              ...
              <select v-model="taskAssignee" class="tabListItemsText" @change="changeAssignee">
                ...
                <option
                  ...
                  v-for="(groupUser, index) in groupPeople"
                  :key="index"
                  :selected="groupUser.assigneeId === assignee.userId"
                  :value="groupUser.assigneeId"
                >{{groupUser.assigneeFirstName}} {{groupUser.assigneeLastName}}</option>
              </select>
            </v-list-item-content>
          </v-list-item>
        </v-list-item-group>
                  ...
                  >
                    ...
                    <v-list-item-content>
                      <v-list-item-title class="subTaskListName">
                        <input
                          maxlength="51"
                          class="subTaskListNameContent"
                          v-model="subtask.subtaskName"
                          type="text"
                          @keyup.enter="subTaskUpdate(subtask)"
                        />
                    ...
                    <v-icon
                      color="#FF6161"
                      @click="selectSubTask(subtask,index); subTaskDialog = true"
                    >mdi-trash-can-outline</v-icon>
                  </v-list-item>
                  <v-list-item v-if="showNewSubTask" class="subTaskListItems">
                    ...
                    <div>
                      <v-list-item-title class="subTaskListName">
                        <input
                          maxlength="51"
                          class="subTaskListNameContent addSubTaskTextbox"
                          placeholder="Add new"
                          v-model="newSubTask.subtaskName"
                          type="text"
        ...
        <v-list-group>
          ...
          <div class="attchmentContainer">
            <v-list-item
              class="subTaskListItems"
              v-for="(taskFile,index) in taskFiles"
              :key="index"
            >
              ...
              <div class="attachmentClose">
                <v-icon
                  @click="removeFiles(taskFile.taskFileId)"
                  size="15"
                  class="closeButton"
                  color="red"
...
export default {
  ...
  methods: {
    ...
    async removeFiles(taskFile) {
      ...
      try {
        response = await this.$axios.$delete(
          `/projects/${this.group.taskGroupId}/tasks/${this.task.taskId}/upload/${taskFile}`,
          {
            data: {},
            headers: {
              user: this.userId
    ...
    async deleteTask() {
      ...
      try {
        ...
        this.$store.dispatch(
          "groups/groupTask/removeTaskFromTaskGroup",
          this.task.taskId
        );
      ...
      if (this.newSubTask.subtaskName) {
        ...
        try {
          response = await this.$axios.$post(
            `/projects/${this.group.taskGroupId}/tasks/${this.task.taskId}/subtask`,
            {
              taskId: this.task.taskId,
    ...
    async subTaskUpdate(editsubtask) {
      if (editsubtask.subtaskName) {
        ...
        try {
          response = await this.$axios.$put(
            `/projects/${this.group.taskGroupId}/tasks/${this.task.taskId}/subtask/${editsubtask.subtaskId}`,
            {
    ...
    async deleteSubTask() {
      ...
      try {
        response = await this.$axios.$delete(
          `/projects/${this.group.taskGroupId}/tasks/${this.task.taskId}/subtask/${this.selectedSubTask.subtaskId}`,
          {
            headers: {
              user: this.userId,
              type: "taskGroup"
    ...
    taskFileUpload() {
      ...
      this.$axios
        .$post(
          `/projects/${this.group.taskGroupId}/tasks/${this.task.taskId}/upload`,
          formData,
          {
            headers: {
  ...
  computed: {
    ...
    taskRemindOn: {
      get() {
        if (this.task.taskReminderAt == null) return "Add Reminder Date";
