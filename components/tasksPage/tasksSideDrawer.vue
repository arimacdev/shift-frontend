  ...
  methods: {
    ...
    async removeFiles(taskFile) {
      ...
      try {
        response = await this.$axios.$delete(
          `/non-project/tasks/personal/${this.task.taskId}/files/${taskFile}`,
          {
            data: {},
            headers: {
              user: this.userId,
      ...
      if (this.newSubTask.subtaskName) {
        ...
        try {
          response = await this.$axios.$post(
            `/non-project/tasks/personal/${this.task.taskId}/subtask`,
            {
              taskId: this.task.taskId,
    ...
    async subTaskUpdate(editsubtask) {
      if (editsubtask.subtaskName) {
        ...
        try {
          response = await this.$axios.$put(
            `/non-project/tasks/personal/${this.task.taskId}/subtask/${editsubtask.subtaskId}`,
            {
