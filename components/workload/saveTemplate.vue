  ...
  <div class="text-center adasd">
    <v-dialog scrollable v-model="dialog" width="600">
      <template v-slot:activator="{ on }">
        <v-btn depressed width="100%" color="#0083E2" dark v-on="on"
      ...
      <v-card height="100%">
        <v-form v-model="isValid" ref="form">
          <v-card-title class="grey lighten-2" primary-title
            >Save filteration as a template</v-card-title
          >
          <div class="saveTemplateBody">
            <v-row>
              <v-col>
                ...
                <span
                  v-for="(assignee, index) in this.filterAssignee"
                  :key="index"
                >
                  {{ assignee.name }}
                  &nbsp;&nbsp;
            ...
            <v-row>
              <v-col>
                ...
                <span
                  v-for="(project, index) in this.filterProject"
                  :key="index"
                >
                  {{ project.name }}
                  &nbsp;&nbsp;
